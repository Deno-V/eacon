...
def easy_verify(atomicclaim,claim,evidence):
    ret = api.send_message(format_chat(['','Given golden evidence:{evidence}.\n In the saying of {claim}. Based on the golden evidence. Is it true that {atomicclaim}? (Answer in one word, you should be strict, if the saying is not matching golden evidence in any detail, you should deny it by saying no.)'.format(evidence=evidence,claim=claim,atomicclaim=atomicclaim)]))
    ret = ret.lower()
    if 'yes' in ret:
        return True
    else:
...
def easy_verifyfs(atomicclaim,claim,evidence):
    ret = api.send_message(format_chat(['','Given golden evidence:{evidence}.\n Based on the golden evidence. Is it true that {atomicclaim}? (Answer in one word, you should be strict, if the saying is not matching golden evidence in any detail, you should deny it by saying no.)'.format(evidence=evidence,claim=claim,atomicclaim=atomicclaim)]))
    ret = ret.lower()
    if 'yes' in ret:
        return True
    else:
...
format_chat = format_vicuna_chat_input
if not fs:
    dataset = load_hover_dataset(path='../testdata/dev_vicunaextract_filtered.json')
else:
    dataset = load_fs_dataset(path='../testdata/fsdev_vicunaextract_filtered.json')
...
for id in xxxx:
    ...
    if not fs:
        f = open('VicunaOutputRecord.txt','a')
    else:
        f = open('fsVicunaOutputRecord.txt','a')
    f.write('*'*30+'\n')
    f.write('id:'+str(id)+'\n')
    f.write('  Claim: '+claim+'\n')
